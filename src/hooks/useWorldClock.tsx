...
const DEFAULT_CITY_IDS = ["seattle", "seoul", "london", "sydney"];

export function WorldClockProvider({
  children,
}: {
  ...
  const [timelineHighlightColor, setTimelineHighlightColorState] =
    useState<string>(() => {
  ...
  const [timelineMode, setTimelineModeState] = useState<TimelineMode>(() => {
    return (
      (localStorage.getItem("worldclock-timeline-mode") as TimelineMode) ||
      "default"
    );
  });
